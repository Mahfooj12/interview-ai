...
import { LiveTranscript } from "@/components/interview/voice/LiveTranscript";
...
export function VideoInterviewPanel() {
  ...
  const autoListen = useAutoListen({
    ...
    onDone: (reason) => {
      console.log(`[Video] auto-listen done (${reason})`);
  ...
  const beginListening = useCallback(() => {
    if (!voice.speechSupported) {
      toast.error("Your browser does not support live transcription. Please use Chrome or Edge.");
      return;
    }
    if (autoListen.isActive) return;
    console.log("[Video] begin listening");
  ...
  const handleSubmitAnswer = useCallback(
    async (reason: "silence" | "maxTime" | "manual") => {
      ...
      const transcript = (voice.liveTranscript || voice.transcript || "").trim();
      console.log(`[Video] submitting (${reason}):`, transcript);
        ...
        <div className="space-y-4 lg:col-span-2">
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="sm:col-span-2">
              <AvatarStage
                ...
                listening={autoListen.isActive}
              />
            </div>
            <div className="space-y-3">
              <CameraPreview enabled={cameraOn} />
              <Button
                size="sm"
                variant="outline"
                className="w-full"
          ...
          <div className="flex flex-col items-center gap-2 rounded-lg border bg-card p-4">
            <Button
              ...
              disabled={
                voice.submitting || voice.speaking || iv.isThinking || !voice.question
              }
